import React from 'react'
import "./NetworkRight.css"

import LinkIcon from '@material-ui/icons/Link';
import PeopleIcon from '@material-ui/icons/People';

import ExitToAppIcon from '@material-ui/icons/ExitToApp';
import CloseIcon from '@material-ui/icons/Close';

function NetworkRight() {
    return (
        <div className="">
        <div className="network__right">
            <div className="network__right__title">
                    <h3>Invitations</h3>
            </div>

            {/* Invitations */}
            <div className="network__right__list">
                <div className="nrl__dp">
                    <img src="https://media-exp1.licdn.com/dms/image/C5603AQFPQ7isquEENw/profile-displayphoto-shrink_100_100/0/1596392284467?e=1627516800&v=beta&t=y_AnrUTZT_N3tjS0L-9xfsq64PJZ0b2-0EX0KyJ6skU" alt="" />
                </div>
                <div className="nrl__info">
                        <h3>Abhay kumar</h3>
                        <h4>Loreconsectetur adipisicing elit. Porro a eligendi velit itaque quidem corporis.</h4>
                       <div className="nrl__info__p"> <LinkIcon className="nrl__link"/>  <p>  0 connections</p></div> 
                </div>

                <div className="nrl__action">
                    <button className="nrl__ignore">Ignore</button> 
                    <button className="nrl__accept">Accept</button> 
                </div>

            </div>


        </div>




        {/* Groups */} 

        <div className="network__right">
            <div className="network__right__title">
                    <h3>Groups</h3>
            </div>

            {/* Invitations */}
            <div className="network__right__list">
                <div className="nrl__dp__group">
                    <img src="https://media-exp1.licdn.com/dms/image/C5603AQFPQ7isquEENw/profile-displayphoto-shrink_100_100/0/1596392284467?e=1627516800&v=beta&t=y_AnrUTZT_N3tjS0L-9xfsq64PJZ0b2-0EX0KyJ6skU" alt="" />
                </div>
                <div className="nrl__info">
                        <h3>Abhay kumar</h3>
                        <h4>Loreconsectetur adipisicing elit. Porro a eligendi velit itaque quidem corporis.</h4>
                       <div className="nrl__info__p"> <PeopleIcon className="nrl__link"/>  <p>  0 members</p></div> 
                </div>

                <div className="nrl__action nrl__delete__wrapper">
                    <ExitToAppIcon  className="nrl__delete"/>  
                   
                </div>

            </div>


        </div>




        {/* Groups */} 

        <div className="network__right">

            
            <div className="network__right__title">
                    <h3>Pending Requests</h3>
            </div>

            <div className="nrl__rc__grid">

                        <div className="nrl__rc__griditem">

                                <div className="nrl__grid__close">
                                    <div className="nrl__grid__close__wrapper">
                                        <CloseIcon className="nr__close__icon"/>
                                    </div>
                                </div>

                                <div className="nrl__img__bg">
                                    <img src="data:image/jpeg;base64," alt="" />
                                </div>
                                <img src="https://media-exp1.licdn.com/dms/image/C5603AQFPQ7isquEENw/profile-displayphoto-shrink_100_100/0/1596392284467?e=1627516800&v=beta&t=y_AnrUTZT_N3tjS0L-9xfsq64PJZ0b2-0EX0KyJ6skU" alt="" />
                                <div className="nr__tc">
                                <h3>Harshal Patel</h3>
                                <h4>SDE</h4>
                                <p>19 Followers</p>
                                 
                                </div>
                                <button>Connect</button>

                        </div>
                        <div className="nrl__rc__griditem">

                                <div className="nrl__grid__close">
                                    <div className="nrl__grid__close__wrapper">
                                        <CloseIcon className="nr__close__icon"/>
                                    </div>
                                </div>

                                <div className="nrl__img__bg">
                                    <img src="data:image/jpeg;base64," alt="" />
                                </div>
                                <img src="https://media-exp1.licdn.com/dms/image/C5603AQFPQ7isquEENw/profile-displayphoto-shrink_100_100/0/1596392284467?e=1627516800&v=beta&t=y_AnrUTZT_N3tjS0L-9xfsq64PJZ0b2-0EX0KyJ6skU" alt="" />
                                <div className="nr__tc">
                                <h3>Harshal Patel</h3>
                                <h4>SDE</h4>
                                <p>19 Followers</p>
                                 
                                </div>
                                <button>Connect</button>

                        </div>
                        <div className="nrl__rc__griditem">

                                <div className="nrl__grid__close">
                                    <div className="nrl__grid__close__wrapper">
                                        <CloseIcon className="nr__close__icon"/>
                                    </div>
                                </div>

                                <div className="nrl__img__bg">
                                    <img src="data:image/jpeg;base64," alt="" />
                                </div>
                                <img src="https://media-exp1.licdn.com/dms/image/C5603AQFPQ7isquEENw/profile-displayphoto-shrink_100_100/0/1596392284467?e=1627516800&v=beta&t=y_AnrUTZT_N3tjS0L-9xfsq64PJZ0b2-0EX0KyJ6skU" alt="" />
                                <div className="nr__tc">
                                <h3>Harshal Patel</h3>
                                <h4>SDE</h4>
                                <p>19 Followers</p>
                                 
                                </div>
                                <button>Connect</button>

                        </div>
                        <div className="nrl__rc__griditem">

                                <div className="nrl__grid__close">
                                    <div className="nrl__grid__close__wrapper">
                                        <CloseIcon className="nr__close__icon"/>
                                    </div>
                                </div>

                                <div className="nrl__img__bg">
                                    <img src="data:image/jpeg;base64," alt="" />
                                </div>
                                <img src="https://media-exp1.licdn.com/dms/image/C5603AQFPQ7isquEENw/profile-displayphoto-shrink_100_100/0/1596392284467?e=1627516800&v=beta&t=y_AnrUTZT_N3tjS0L-9xfsq64PJZ0b2-0EX0KyJ6skU" alt="" />
                                <div className="nr__tc">
                                <h3>Harshal Patel</h3>
                                <h4>SDE</h4>
                                <p>19 Followers</p>
                                 
                                </div>
                                <button>Connect</button>

                        </div>
                        <div className="nrl__rc__griditem">

                                <div className="nrl__grid__close">
                                    <div className="nrl__grid__close__wrapper">
                                        <CloseIcon className="nr__close__icon"/>
                                    </div>
                                </div>

                                <div className="nrl__img__bg">
                                    <img src="data:image/jpeg;base64," alt="" />
                                </div>
                                <img src="https://media-exp1.licdn.com/dms/image/C5603AQFPQ7isquEENw/profile-displayphoto-shrink_100_100/0/1596392284467?e=1627516800&v=beta&t=y_AnrUTZT_N3tjS0L-9xfsq64PJZ0b2-0EX0KyJ6skU" alt="" />
                                <div className="nr__tc">
                                <h3>Harshal Patel</h3>
                                <h4>SDE</h4>
                                <p>19 Followers</p>
                                 
                                </div>
                                <button>Connect</button>

                        </div>

            </div>



        </div>




        

        <div className="network__right">


            <div className="network__right__title">
                    <h3>Recommendations</h3>
            </div>

            <div className="nrl__rc__grid">

                        <div className="nrl__rc__griditem">

                                <div className="nrl__grid__close">
                                    <div className="nrl__grid__close__wrapper">
                                        <CloseIcon className="nr__close__icon"/>
                                    </div>
                                </div>

                                <div className="nrl__img__bg">
                                    <img src="data:image/jpeg;base64," alt="" />
                                </div>
                                <img src="https://media-exp1.licdn.com/dms/image/C5603AQFPQ7isquEENw/profile-displayphoto-shrink_100_100/0/1596392284467?e=1627516800&v=beta&t=y_AnrUTZT_N3tjS0L-9xfsq64PJZ0b2-0EX0KyJ6skU" alt="" />
                                <div className="nr__tc">
                                <h3>Harshal Patel</h3>
                                <h4>SDE</h4>
                                <p>19 Followers</p>
                                 
                                </div>
                                <button>Connect</button>

                        </div>

            </div>



        </div>







     </div>
    )
}

export default NetworkRight
